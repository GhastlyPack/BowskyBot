      ...
      <header className="border-b border-neutral-800">
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <p className="text-neutral-400 mt-2">
            Frameworks, templates, SOPs, and tools for community members
          </p>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((cat) => (
            <Link
              key={cat.slug}
              href={`/${cat.slug}`}
              className="group block rounded-xl border border-neutral-800 bg-neutral-900/50 p-6 hover:border-neutral-600 hover:bg-neutral-900 transition-all"
            >
              <div className="flex items-start justify-between mb-4">
                <span className="text-3xl">{cat.icon}</span>
                <span className={`text-xs px-2 py-1 rounded-full border ${tierColors[cat.tier]}`}>
                  {tierLabels[cat.tier]}
                </span>
              </div>
      ...
      <footer className="border-t border-neutral-800 mt-12">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <p className="text-sm text-neutral-500">
            Bowsky's Community &mdash; New resources are added regularly. Check the Discord for announcements.
          </p>
        </div>
      </footer>
    </div>
